...
| 상황 | 출력 예시 |
|---|---|
| diff/커밋 확인 시작 | `⏺ [review] git diff HEAD~1 분석 중…` |
| 파일 상세 검토 시작 | `⏺ [review] WordCardViewModel.kt 검토 중…` |
| PRD 교차 확인 | `⏺ [review] docs/prd/04-feature-flashcard.md AC 대조 중…` |
| 회귀 위험 점검 | `⏺ [review] Navigation/Room 회귀 위험 점검 중…` |
| 리뷰 보고서 작성 | `⏺ [review] 리뷰 보고서 작성 중…` |
