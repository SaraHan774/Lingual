...
- **AC 작성 프리플라이트 체크리스트** — prd iter 1 에서 아래 5개 카테고리가 누락되어 prd-review 가 NEEDS_SPEC 으로 반려한 사례 재발 3건(`flashcard-ui-ux`, `diary-edit`, `wordcard-translation-edit`). 새 기능 / 변경 기능 PRD 를 방출하기 전에 각 항목이 AC 또는 UX Flow 에 명시되었는지 self-check 한다.
  1. **에러 상태**: 네트워크/번역/DB 실패, 권한 거부, 타임아웃 등 각 주요 실패 경로의 UX(스낵바 문구, BottomSheet 유지 여부, 재시도 경로).
  2. **엣지 케이스**: 빈 입력, 초과 길이, 번역 전무, 미설치 TTS Locale, 롱탭/스와이프 충돌, 매우 긴 콘텐츠 스크롤.
  3. **파괴적 동작**: 덮어쓰기·삭제·무효화가 복원 불가이면 사용자 고지 문구 + 확인 다이얼로그 정책을 AC 로 명문화.
  4. **Open Questions 해소**: 구현과 직결되는 결정(예: 아이콘 위치, 필터 순서, 필드 복원 여부)은 "Open" 으로 남기지 말고 **결정 섹션**으로 옮긴다. 미결정인 채 code 단계로 내려가면 back-prop 원인.
  5. **내부 일관성**: 같은 문서 내 필드 정의(Data Model)·AC·UX Flow 가 서로 충돌하지 않는지(예: "Phase 2 미사용" 필드를 v0.1 AC 가 참조하는 상황) 재확인.
